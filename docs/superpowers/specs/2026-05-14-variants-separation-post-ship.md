...
- **Transient `RangeError: Maximum call stack size exceeded` in `buildImageParts`** — observed once on 2026-05-14 right after the variants-separation merge, NOT reproducible after a `npm run dev` restart. Stack pointed at `lib/providers/comfy.ts::buildImageParts → uploadSingleImage → uploadBinaryToSignedUrl`. None of these are recursive; suspect a V8 optimizer hiccup or a deep stream pipe inside Node's fetch on a particular signed-URL response. Codepath was NOT touched by this branch — pre-existing input-image upload path for Comfy. If it recurs, capture: (a) the input image dimensions and file size, (b) full server stack trace, (c) Node version. The proven retry pattern is already in `fetchWithRetry` but only handles network errors and retriable HTTP statuses, not JS `RangeError`. Don't add a `catch (RangeError)` retry without first reproducing — that's a hypothesis fix per `feedback_dont_withhold_defensive_fixes` semantics (the rule allows defensive cleanup that copies a proven pattern, not blind hypothesis fixes).
